...
*   **Meta Conversions API (`lib/metaCapi.ts`):** Server-side Purchase/InitiateCheckout/AddToCart events via FB Graph `/vN/pixelId/events`. Reads pixel config from `marketing_integrations` (platform="facebook"). All PII SHA-256 hashed. `fireCapiPurchase()` called fire-and-forget on every order creation and JazzCash/Easypaisa callback. Bypasses browser ad-blockers and iOS restrictions for accurate attribution.
*   **JazzCash/Easypaisa (`lib/jazzcash.ts`, `lib/easypaisa.ts`, `routes/onlinePayments.ts`):** Real hosted-checkout payment flow. After order creation, frontend submits a hidden form POST to JazzCash or POSTs to Easypaisa API then window.location redirects. Callbacks at `/api/payments/jazzcash/callback` and `/api/payments/easypaisa/callback` verify HMAC, mark order paid, fire CAPI. Browser return redirects to `/order-success` or `/checkout?error=…`. Gateway config from `payment_gateways` table (apiKey=merchantId/storeId, secretKey=password/hashKey, webhookSecret=integrationSalt).
*   **Storefront SSR (`kdf-nuts/src/entry-server.tsx`, `api-server/src/lib/ssrMiddleware.ts`):** Product pages rendered server-side for Googlebot. `entry-server.tsx` uses Vite SSR + wouter `memoryLocation` hook to render `/products/:slug` and `/products` to HTML string with injected Product schema.org JSON-LD, canonical, OG tags, and dehydrated TanStack Query state. `ssrMiddleware.ts` prefetches product from DB, injects into HTML shell, caches 5 min. Active only in production for `khanbabadryfruits.com` hostname. SSR build: `pnpm --filter @workspace/kdf-nuts run build:ssr`.
...
*   **Courier API Coverage:** TCS, PostEx, Leopards, Trax, **M&P** (`mp`), **BlueEX** (`bluex`), **Call Courier** (`callcourier`), **Swyft** (`swyft`). All 8 wired into `callCourierApi()`, `trackWithCourierApi()`, `getTrackingUrl()`, and `generateTrackingId()` in `routes/couriers.ts`. Add new courier slug + credentials in Admin → Couriers to activate.
*   **TCS ECOM API:** Uses `ociconnect.tcscourier.com/ecom/api`. IBM API Connect gateway — Bearer token required on **ALL** endpoints (auth + booking). Auth: `GET /authentication/token?username=&password=` with headers `Authorization: Bearer {bearerToken}` + `X-IBM-Client-Id: {username}` → returns `accesstoken` (lowercase, URL-encoded — must `decodeURIComponent()`). Booking: `POST /booking/create` with same Bearer headers + body `{ accesstoken, shipperinfo: { tcsaccount: username }, consigneeinfo, vendorinfo, shipmentinfo }`. Key field rules: `weightinkg` and `skus[0].weight` must be **decimal string** e.g. `"0.50"` (not number). `servicecode` must be letter **`"O"`** (Overnight), `"E"` (Economy), `"S"` (Same-day) — NOT digits. City: use `normalizeCity()` to strip DHA/Phase/Cantt/sector prefixes; 40+ Pakistani cities mapped in `CITY_CODES`; unknown cities send empty code + raw name. City list API: `GET /ecom/api/cities` → `{ city: [{ cityID, cityName, cityCode, area }] }`. Admin endpoint: `GET /api/admin/couriers/tcs/cities`. Validation catches "Select City" placeholder before booking. Cost Center Code = first number in Pickup Address (e.g. "999"). Implementation: `artifacts/api-server/src/lib/tcs.ts`.
