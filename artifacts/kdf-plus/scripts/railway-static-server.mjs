...
/** See kdf-admin static server — same misroute pattern if `api.*` hits storefront by mistake. */
function corsHeadersForApiMisroute(req) {
  const origin = req.headers.origin;
  const requestHdrs = req.headers["access-control-request-headers"];
  const headers = {
    "X-KDF-Backend": "kdf-plus-static",
    ...
    res
      ...
      .end(
        JSON.stringify({
          ...
          message:
            "This host serves static files only. Point api.<your-domain> at @workspace/api-server on Railway, not kdf-plus.",
          service: "kdf-plus",
        }),
      );
    return;
