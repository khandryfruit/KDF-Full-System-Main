...
function RiderCard({ rider, onToggle, onAssign }: { rider: any; onToggle: () => void; onAssign: () => void }) {
...
export default function AdminRiders() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setRiders(prev => prev.map(r =>
          r.id === rider.id ? { ...r, is_online: !rider.is_online } : r
        ));
      }
    } catch {}
  };
  ...
  const autoAssign = async () => {
    ...
    setLastResult(null);
    try {
      const res = await adminFetch("/admin/riders/auto-assign", token, {
        method: "POST",
  ...
  return (
    <View style={[styles.root, { paddingTop: insets.top }]}>
      ...
      {lastResult ? (
        <View style={[styles.resultBanner, { backgroundColor: lastResult.startsWith("✅") ? `${GREEN}18` : `${RED}18`, borderColor: lastResult.startsWith("✅") ? `${GREEN}30` : `${RED}30` }]}>
