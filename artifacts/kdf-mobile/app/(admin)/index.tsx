...
function KpiCard({
  ...
  return (
    <TouchableOpacity
      ...
      style={[styles.kpi, { borderColor: `${color}22` }]}
    >
        ...
        {loading ? (
          ...
          <>
            ...
            <View style={styles.kpiGrid}>
              ...
              <KpiCard icon="check-circle" label="Delivered Today" value={stats.delivered_today ?? 0}
                color={BLUE} />
              <KpiCard icon="dollar-sign" label="COD Today"
                value={`Rs.${Number(stats.cod_collected_today ?? 0).toLocaleString()}`}
                color={GOLD} />
            </View>
            ...
            {riders.length > 0 && (
              <View style={styles.card}>
                <View style={styles.cardHeader}>
                  ...
                  <TouchableOpacity onPress={() => router.push("/(admin)/riders" as any)} style={styles.seeAll}>
            ...
            <View style={styles.card}>
              ...
              <View style={styles.actionsGrid}>
                <QuickAction icon="navigation" label="Auto-Assign Orders" color={GREEN}
                  onPress={() => router.push("/(admin)/riders" as any)} />
                <QuickAction icon="sliders" label="Module Controls" color="#A78BFA"
                  onPress={() => router.push("/(admin)/modules" as any)} />
                <QuickAction icon="shopping-bag" label="View Orders" color={BLUE}
                  onPress={() => router.push("/(admin)/orders" as any)} />
                <QuickAction icon="user" label="My Profile" color={GOLD}
...
function getStatusColor(s: string) {
  const map: Record<string, string> = {
    ...
    out_for_delivery: "#8B5CF6", failed: "#EF4444", returned: "#F97316",
    near_customer: "#06B6D4",
  };
  return map[s] ?? "#6B7280";
}
...
const styles = StyleSheet.create({
  ...
  liveIndicator: {
    ...
    backgroundColor: "rgba(16,185,129,0.1)", paddingHorizontal: 10, paddingVertical: 5,
  ...
  card: {
    marginHorizontal: 16, marginBottom: 14, backgroundColor: CARD,
    borderRadius: 20, padding: 16, borderWidth: 1, borderColor: BORDER,
  },
