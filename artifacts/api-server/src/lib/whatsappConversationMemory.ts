...
export async function loadConversationMemory(phone: string) {
  ...
  return {
    ...
    selectedProductName: String(stateData.selectedProductName ?? stateData.product?.name ?? stateData.lastOfferedProduct?.name ?? ""),
    selectedVariantTitle: String(stateData.selectedVariantTitle ?? stateData.variantTitle ?? ""),
    lastOfferedProduct: stateData.lastOfferedProduct ?? stateData.product ?? null,
