...
async function createCountdownBanner(title: string, payload: Record<string, unknown>) {
  ...
  const result = await db.execute(sql`
    INSERT INTO ${bannersTable} (
      ...
      "bg_color", "text_color", "label", "cta", "platform", "placement",
      "sort_order", "active", "countdown_end_at", "start_date", "end_date",
      "offer_product_ids", "offer_category_ids", "offer_mode",
      "offer_display_count", "offer_sort", "show_timer",
      "button_bg_color", "button_text_color"
    ) VALUES (
      ${title},
      ${textOrNull(payload.subtitle)},
      ${textOrNull(payload.imageUrl)},
      ${textOrNull(payload.mobileImageUrl)},
      ${textOrNull(payload.linkUrl) ?? "/products"},
      ${textOrNull(payload.bgColor) ?? "#0D2B00"},
      ${textOrNull(payload.textColor) ?? "white"},
      ${textOrNull(payload.label)},
      ${textOrNull(payload.cta) ?? "Shop Now"},
      ${textOrNull(payload.platform) ?? "both"},
      'countdown_deal',
      ${intValue(payload.sortOrder, 0)},
      ${payload.active !== false},
      ${dateOrNull(payload.countdownEndAt)},
      ${dateOrNull(payload.startDate)},
      ${dateOrNull(payload.endDate)},
      CAST(${JSON.stringify(idArray(payload.offerProductIds))} AS jsonb),
      CAST(${JSON.stringify(idArray(payload.offerCategoryIds))} AS jsonb),
      ${textOrNull(payload.offerMode) ?? "discount_products"},
      ${Math.max(1, Math.min(12, intValue(payload.offerDisplayCount, 8)))},
      ${textOrNull(payload.offerSort) ?? "featured"},
      ${payload.showTimer !== false},
      ${textOrNull(payload.buttonBgColor)},
      ${textOrNull(payload.buttonTextColor)}
    )
    RETURNING *
  `);
