...
import { sendWhatsAppMessage, sendWhatsAppTemplate, sendInteractiveMenu, sendInteractiveButtons, sendInteractiveList, sendCtaUrlMessage, normalizePhone, getConversationState, setConversationState, isGreeting, markWhatsAppMessageRead, sendWhatsAppTypingIndicator, getSettings } from "../lib/whatsapp";
...
async function routeUnifiedCustomerSupport(opts: {
  ...
  const handled = await tryHandleClassifiedSupport({
    ...
    sendText: async (p, text, template) => {
      await sendWaReplyWithActions(p, text, opts.waSettings, {
    ...
    for (const entry of (body.entry ?? [])) {
      for (const change of (entry.changes ?? [])) {
        ...
        for (const msg of (value.messages ?? [])) {
          ...
          if (contactName) {
            try {
              const sd = JSON.parse((convState as any)?.stateData ?? "{}");
              if (!sd.waContactName) {
                sd.waContactName = contactName;
                await setConversationState(phone, currentState, sd);
              }
            } catch { /* ignore */ }
          }
          ...
          if (msgType === "interactive" && interactionId) {
            ...
            if (interactionId === "wa_chat_order_confirm" || interactionId === "wa_chat_order_cancel") {
              ...
              try {
                const flowText =
                  currentState === "wa_order_await_preconfirm"
                    ? (interactionId === "wa_chat_order_confirm" ? "1" : "2")
          ...
          const { WA_ORDER_AWAIT_BANK_SCREENSHOT, WA_ORDER_AWAIT_EASYPAISA_SCREENSHOT } = await import("../lib/waPremiumJourney.js");
          if (
            (currentState === WA_ORDER_AWAIT_BANK_SCREENSHOT || currentState === WA_ORDER_AWAIT_EASYPAISA_SCREENSHOT) &&
          ...
          const {
            WA_ORDER_AWAIT_BANK_SCREENSHOT: waBankShot,
            WA_ORDER_AWAIT_EASYPAISA_SCREENSHOT: waEasyShot,
...
/** Human text first, then context-aware quick-action template (hybrid reply). */
async function sendWaReplyWithActions(
  phone: string,
  message: string,
  waSettings: any,
  opts: {
    templateName?: string;
    skipActions?: boolean;
    actionContext?: import("../lib/waQuickActions.js").QuickActionContext;
    textBody?: string;
    classified?: import("../lib/waIntentClassifier.js").ClassifiedMessage;
    intent?: string;
    stateData?: Record<string, unknown>;
    currentState?: string;
  } = {},
): Promise<string | null> {
  const msgId = await sendWaText(phone, message, waSettings, opts.templateName ?? "bot_reply");
  if (opts.skipActions) return msgId;
  const conv = await getConversationState(phone).catch(() => null);
  const { attachQuickActions } = await import("../lib/waQuickActions.js");
  await attachQuickActions({
    ...
    currentState: opts.currentState ?? conv?.state,
    context: opts.actionContext,
  }).catch(() => {});
  return msgId;
}
...
async function respondWithFreshProductSearch(opts: {
  ...
  const shown = await deliverSingleProductOffer({
    ...
    query: q,
    waSettings: opts.waSettings,
    showMore: isShowMoreProductsMessage(q),
  });
  ...
  if (!shown) {
    ...
    await sendWaText(
      opts.phone,
      roman
        ? `Ji 😊 "${q}" ke liye abhi product nahi mila. *badam*, *pista*, *kaju* try karein 😊`
...
async function tryResolveWaCustomerMessage(opts: {
  ...
  if (opts.currentState === WA_AWAIT_LANGUAGE_STATE) {
    ...
    if (lang) {
      ...
      await sendWaReplyWithActions(opts.phone, buildHumanWelcomeText(opts.textBody, lang), opts.waSettings, {
        ...
        actionContext: "greeting",
        textBody: opts.textBody,
        stateData: { preferredLanguage: lang, waLang: lang },
      });
      await persistConversationTurn(opts.phone, { intent: "greeting", topic: "greeting_text", mergeStateData: { preferredLanguage: lang } });
      return true;
    }
  ...
  if (opts.currentState === "wa_order_await_variant" && isShowMoreProductsMessage(opts.textBody)) {
    ...
    try { data = JSON.parse((conv as any)?.stateData ?? "{}"); } catch { /* ignore */ }
    await deliverSingleProductOffer({
      phone: opts.phone,
      query: data.productQuery ?? opts.textBody,
      waSettings: opts.waSettings,
      showMore: true,
    });
    return true;
  ...
  if (convReply.handled) {
    ...
    if (convReply.template === "standalone_price_text" && convReply.reply) {
      await sendWaReplyWithActions(opts.phone, convReply.reply, opts.waSettings, {
        templateName: "standalone_price_text",
    ...
    if (convReply.template === "product_interest_text" && convReply.reply) {
      ...
      const lang = resolveWaLang(stateData, opts.textBody);
      await sendWaReplyWithActions(opts.phone, convReply.reply, opts.waSettings, {
        templateName: "product_interest_text",
        actionContext: "product",
        textBody: opts.textBody,
        stateData: { ...stateData, pendingProductQuery: productQ },
      });
      ...
      await persistConversationTurn(opts.phone, { intent: opts.detectedIntent.intent, topic: "product_interest" });
      return true;
    }
...
async function startCommerceOrderFromText(opts: {
  ...
  if (parsed.variantTitle) {
    ...
    if (found) {
      await setConversationState(opts.phone, "wa_order_await_preconfirm", {
      ...
      await sendWaText(opts.phone, buildPreconfirmMessage({
        productName: found.product.name,
        variantTitle: found.variantTitle,
        unitPrice: found.unitPrice,
        quantity: parsed.quantity,
  ...
  if (await deliverSingleProductOffer({ phone: opts.phone, query: parsed.productQuery, waSettings: opts.waSettings })) {
    return true;
  }
...
async function proceedToOrderConfirmSummary(
  ...
  const { sendOrderConfirmButtons } = await import("../lib/waPremiumUi.js");
...
async function sendPaymentMethodButtons(phone: string, stateData: Record<string, any>, waSettings: any): Promise<void> {
  const { sendPaymentPicker } = await import("../lib/waPremiumUi.js");
  await setConversationState(phone, "wa_order_await_payment", stateData);
  await sendPaymentPicker(phone, stateData, waSettings);
}
...
async function handlePremiumCommerceButton(opts: {
  ...
  const premium = await import("../lib/waPremiumJourney.js");
  const ui = await import("../lib/waPremiumUi.js");
  const {
    parseLanguageChoice,
  ...
  if (id.startsWith("wa_qa_")) {
    ...
    if (id === "wa_qa_payment") {
      ...
      await sendWaReplyWithActions(
        ...
        lang === "en"
          ...
          : "Ji 😊 Payment options:",
        opts.waSettings,
        { templateName: "qa_payment_intro", skipActions: true },
      );
      await paymentChat.sendStandalonePaymentMenu({ phone: opts.phone, lang, waSettings: opts.waSettings });
      return true;
    }

    if (id === "wa_qa_delivery") {
      const { buildDeliveryConversationText } = await import("../lib/waConversationFlows.js");
      await sendWaReplyWithActions(
        opts.phone,
        buildDeliveryConversationText(String(stateData.lastUserMessage ?? "delivery"), lang),
        opts.waSettings,
        { templateName: "qa_delivery", actionContext: "delivery", stateData },
      );
      return true;
    }

    if (id === "wa_qa_track" || id === "wa_qa_orders_track") {
      await sendWaReplyWithActions(
        opts.phone,
        lang === "en"
          ? "Ji 😊 Please send your *order number* or the phone used when you ordered."
          : "Ji 😊 *Order number* ya order wala phone bhej dein.",
        opts.waSettings,
        { templateName: "qa_track_prompt", actionContext: "orders" },
      );
      return true;
    }
    ...
    if (id === "wa_qa_orders") {
      await sendInteractiveList({
        phone: opts.phone,
        body: lang === "en" ? "📋 *Your orders*" : "📋 *Orders*",
        buttonLabel: lang === "en" ? "Orders" : "Orders",
        rows: [
          { id: "wa_qa_orders_track", title: "📦 Track Order", description: "Current status" },
          { id: "wa_qa_orders_past", title: "📜 Past Orders", description: "Order history" },
          { id: "wa_qa_support", title: "📞 Support", description: "Need help?" },
        ],
        settings: opts.waSettings,
        templateName: "qa_orders_menu",
      });
      return true;
    }

    if (id === "wa_qa_orders_status" || id === "wa_qa_orders_past") {
    ...
    if (id === "wa_qa_support") {
      await sendWaReplyWithActions(
        opts.phone,
        lang === "en" ? "Ji 😊 How can we help?" : "Ji 😊 Kaise madad kar sakte hain?",
        opts.waSettings,
        { templateName: "qa_support_intro", actionContext: "support", skipActions: true },
      );
      await sendInteractiveButtons({
        phone: opts.phone,
        text: lang === "en" ? "Contact us:" : "Rabta:",
        buttons: [
          { id: "wa_qa_call", title: "📞 Call" },
          { id: "wa_qa_whatsapp", title: "💬 WhatsApp" },
          { id: "wa_qa_website", title: "🌐 Website" },
        ],
        settings: opts.waSettings,
        templateName: "qa_support_buttons",
      });
      return true;
    }
    ...
    if (id === "wa_qa_call") {
      ...
      await sendWaReplyWithActions(
        ...
        `📞 *Call Khan Dry Fruit*\n\n${KDF_STORE.phone}\n\nDaily 10 AM – 12 AM`,
        opts.waSettings,
        { templateName: "qa_call", actionContext: "default" },
      );
      return true;
    }

    if (id === "wa_qa_whatsapp") {
      const { KDF_STORE } = await import("../lib/waSupportFlows.js");
      const waNum = KDF_STORE.whatsapp.replace(/\D/g, "").replace(/^0/, "92");
      await sendCtaUrlMessage({
        phone: opts.phone,
        text: `💬 WhatsApp: ${KDF_STORE.whatsapp}`,
        buttonText: "💬 Chat on WhatsApp",
        url: `https://wa.me/${waNum}`,
        settings: opts.waSettings,
        templateName: "qa_whatsapp",
      });
      return true;
    }
    ...
    if (id === "wa_qa_website") {
      ...
      await sendCtaUrlMessage({
        phone: opts.phone,
        text: lang === "en" ? "🌐 Visit our website:" : "🌐 Website:",
        buttonText: "🌐 Visit Website",
        url: webUrl,
        settings: opts.waSettings,
        templateName: "qa_website",
      });
      return true;
    }
    ...
    if (id === "wa_qa_prices") {
      ...
      const roman = lang !== "ur";
      const priceReply = await buildTextOnlyPriceReply(q, roman);
      if (priceReply) {
        await sendWaReplyWithActions(opts.phone, priceReply, opts.waSettings, {
          templateName: "qa_prices",
          actionContext: "product",
          skipActions: true,
        });
        await deliverSingleProductOffer({ phone: opts.phone, query: q, waSettings: opts.waSettings });
      } else {
        await sendWaReplyWithActions(
          opts.phone,
          lang === "en" ? `Ji 😊 Which product price? (e.g. ${q})` : `Ji 😊 Kis product ki price? (jaise ${q})`,
          opts.waSettings,
          { templateName: "qa_prices_ask", actionContext: "product" },
        );
      }
      return true;
    }
  ...
  if (id === "wa_conv_shop") {
    ...
    await sendWaText(
      opts.phone,
      lang === "en"
        ? "Ji 😊 Which product are you looking for? For example: *badam*, *pista*, *kaju* 😊"
        : "Ji 😊 Kaun sa product chahiye? Jaise: *badam*, *pista*, *kaju* 😊",
      opts.waSettings,
      "conv_shop_text",
    );
    return true;
  ...
  if (id === "wa_conv_track") {
    ...
    await sendWaText(
      opts.phone,
      lang === "en"
        ? "Ji 😊 Please send your *order number* or phone used for the order."
        : "Ji 😊 *Order number* ya order wala phone bhej dein.",
      opts.waSettings,
      "tracking_text",
    );
  ...
  if (id === "wa_conv_support") {
    ...
    await sendWaText(
      opts.phone,
      lang === "en"
        ? "Ji 😊 I am here to help. Ask about products, prices, delivery, payment, or your order — just type your question 😊"
        : "Ji 😊 Main madad ke liye hoon. Product, price, delivery, payment, ya order — jo poochna ho likh dein 😊",
      opts.waSettings,
  ...
  if (id === "wa_edu_price" || id === "wa_edu_quality" || id === "wa_edu_order") {
    ...
    const { buildProductQualityMessage, buildProductPriceIntro } = await import("../lib/waConversationFlows.js");
    const { buildTextOnlyPriceReply } = await import("../lib/waSalesConversation.js");
    const roman = lang !== "ur";
    ...
    await sendWaText(opts.phone, buildProductPriceIntro(productQ, lang), opts.waSettings, "product_price_intro");
    await deliverSingleProductOffer({ phone: opts.phone, query: productQ, waSettings: opts.waSettings });
    return true;
  }
  ...
  if (id === "wa_lang_ur" || id === "wa_lang_en" || id === "wa_lang_ps") {
    ...
    await setConversationState(opts.phone, "wa_sales_chat", stateData);
    const { buildHumanWelcomeText } = await import("../lib/waConversationFlows.js");
    await sendWaReplyWithActions(opts.phone, buildHumanWelcomeText(undefined, lang), opts.waSettings, {
      templateName: "human_greeting",
      actionContext: "greeting",
      stateData,
    });
  ...
  if (id.startsWith("wa_browse_")) {
    const q = id.replace("wa_browse_", "");
    await deliverSingleProductOffer({ phone: opts.phone, query: q, waSettings: opts.waSettings });
    return true;
  }

  if (id.startsWith("wa_intent_")) {
    const lang = resolveWaLang(stateData);
    const productQ = String(stateData.pendingProductQuery ?? "").trim();
    if (!productQ) return false;
    if (id === "wa_intent_delivery") {
      const { buildDeliveryReply } = await import("../lib/waIntentEngine.js");
      await sendWaText(opts.phone, await buildDeliveryReply(productQ), opts.waSettings, "delivery_info");
      return true;
    }
    if (id === "wa_intent_price") {
      const { buildTextOnlyPriceReply } = await import("../lib/waSalesConversation.js");
      const roman = lang === "en";
      const priceReply = await buildTextOnlyPriceReply(productQ, roman);
      if (priceReply) await sendWaText(opts.phone, priceReply, opts.waSettings, "product_price_text");
      await sendInteractiveButtons({
        phone: opts.phone,
        text: lang === "en" ? "Next step:" : "Agla step:",
        buttons: [
          { id: "wa_intent_order", title: "🛒 Order" },
          { id: "wa_intent_recommend", title: "⭐ Recommend" },
          { id: "main_menu", title: "🏠 Menu" },
        ],
        settings: opts.waSettings,
        templateName: "after_price_actions",
      });
      return true;
    }
    await setConversationState(opts.phone, "idle", { preferredLanguage: lang });
    await deliverSingleProductOffer({ phone: opts.phone, query: productQ, waSettings: opts.waSettings });
    return true;
  }

  const vIdx = parseVariantListId(id);
  if (vIdx != null && opts.currentState === "wa_order_await_variant") {
    const product = stateData.product;
    const options = Array.isArray(product?.variantOptions) ? product.variantOptions : [];
    const selected = options[vIdx];
    if (!product || !selected) return false;
    const lang = resolveWaLang(stateData);
    stateData.variant = selected;
    stateData.variantTitle = selected.title;
    stateData.unitPrice = parseCatalogUnitPrice(selected.price);
    await setConversationState(opts.phone, "wa_order_await_quantity", stateData);
    await sendQuantityPicker({ phone: opts.phone, stateData, lang, waSettings: opts.waSettings });
    return true;
  }

  if (id === "wa_qty_custom" && opts.currentState === "wa_order_await_quantity") {
    const lang = resolveWaLang(stateData);
    stateData.awaitingCustomQty = true;
    await setConversationState(opts.phone, "wa_order_await_quantity", stateData);
    await sendInteractiveList({
      phone: opts.phone,
      body: lang === "en"
        ? "Type quantity (1–99) in your next message — only step needing a number."
        : "Agla message mein quantity (1–99) likh dein:",
      buttonLabel: "Qty",
      rows: [
        { id: "wa_qty_7", title: "📦 7 Packs" },
        { id: "wa_qty_9", title: "📦 9 Packs" },
        { id: "wa_qty_12", title: "📦 12 Packs" },
      ],
      settings: opts.waSettings,
      templateName: "qty_custom_hint",
    });
    return true;
  }

  const qty = parseQuantityChoice(id);
  if (qty != null && opts.currentState === "wa_order_await_quantity") {
    await proceedAfterQuantityPick(opts.phone, stateData, qty, opts.waSettings);
    return true;
  }

  if (id === "wa_chat_order_cancel") {
    await setConversationState(opts.phone, "idle", { preferredLanguage: stateData.preferredLanguage ?? stateData.waLang });
    await sendWaText(opts.phone, "Order cancel ho gaya 😊 Jab order karna ho product naam bhej dein.", opts.waSettings, "order_cancelled");
    return true;
  }

  if (id === "wa_name_use" || id === "wa_name_guest") {
    const { buildPhonePrompt } = premium;
    stateData.customerName = id === "wa_name_guest"
      ? "Guest"
      : String(stateData.waContactName ?? stateData.customerName ?? "Customer").trim();
  ...
  if (id === "wa_name_enter") {
    const lang = resolveWaLang(stateData);
    await setConversationState(opts.phone, "wa_order_await_name", { ...stateData, awaitingNameText: true });
    await sendInteractiveList({
      phone: opts.phone,
      body: lang === "en" ? "Type your full name in the next message (only step requiring text)." : "Agla message mein apna *poora naam* likh dein:",
      buttonLabel: "Help",
      rows: [{ id: "wa_name_guest", title: "👤 Guest Checkout" }],
      settings: opts.waSettings,
      templateName: "name_type_hint",
    });
    return true;
  }
  ...
  if (id === "wa_phone_same" || id === "wa_phone_other") {
    ...
    if (id === "wa_phone_other") {
      ...
      await sendInteractiveList({
        phone: opts.phone,
        body: lang === "en" ? "Type phone in next message OR pick:" : "Agla message mein number likh dein:",
        buttonLabel: "Phone",
        rows: [{ id: "wa_phone_same", title: "✅ Use WhatsApp No" }],
        settings: opts.waSettings,
        templateName: "phone_type_hint",
      });
      return true;
    }
    stateData.customerPhone = normalizeCheckoutPhone(opts.phone);
  ...
  if (id === "wa_pay_cod" || id === "wa_pay_bank" || id === "wa_pay_easypaisa") {
  ...
  if (id === "wa_payment_help") {
    await sendWaText(opts.phone, "Ji 😊 team madad karegi — call: 0304-9996000", opts.waSettings);
    return true;
  }
  ...
  if (id === "wa_order_edit") {
    ...
    await sendNamePicker({ phone: opts.phone, contactName: stateData.waContactName, lang, waSettings: opts.waSettings });
    return true;
  }
...
async function createEcommerceOrderFromWhatsApp(phone: string, rawStateData: Record<string, any>) {
  ...
  const result = await (db as any).transaction(async (tx: any) => {
    ...
    const [order] = await tx.insert(ordersTable).values({
      ...
      discount: Number(stateData.discount ?? 0).toFixed(2),
...
async function handleCommerceOrderFlow(phone: string, text: string, state: string, waSettings: any, log?: any): Promise<void> {
  ...
  if (state === "wa_order_await_product_choice") {
    ...
    if (!selected) {
      ...
      if (productRootsInMessage(freshQuery).length > 0) {
        await deliverSingleProductOffer({ phone, query: freshQuery, waSettings });
        return;
      }
      await sendWaText(phone, `Ji 😊 option samajh nahi aaya. List se *number* reply karein (1, 2, 3) ya product naam dobara bhej dein.`, waSettings);
      return;
    }
    const { resolveCommerceImageUrl } = await import("../lib/commerceProductSearch.js");
  ...
  if (state === "wa_order_await_preconfirm") {
    ...
    const { resolveWaLang } = await import("../lib/waPremiumJourney.js");
    const { sendQuantityPicker } = await import("../lib/waPremiumUi.js");
    const lang = resolveWaLang(stateData);
    ...
    await sendQuantityPicker({ phone, stateData: { ...stateData, product, variant }, lang, waSettings });
    return;
  }
  if (state === "wa_order_await_variant") {
    const product = stateData.product;
    const options = Array.isArray(product?.variantOptions) ? product.variantOptions : [];
    const { parseVariantListId, resolveWaLang } = await import("../lib/waPremiumJourney.js");
    const { sendVariantPicker, sendQuantityPicker } = await import("../lib/waPremiumUi.js");
    const lang = resolveWaLang(stateData, stateData.lastUserMessage);
    const listIdx = parseVariantListId(trimmed.startsWith("wa_") ? trimmed : "");
    const selectedIndex = listIdx ?? (Number.parseInt(trimmed.replace(/[^0-9]/g, ""), 10) - 1);
    ...
    if (!product || !selected) {
      ...
      if (options.length) {
        ...
        await sendVariantPicker({
          phone,
          productName: product.name,
          variantOptions: options.map((v: any) => ({
            id: String(v.id),
            title: String(v.title),
            price: parseCatalogUnitPrice(v.price),
          })),
    ...
    await setConversationState(phone, "wa_order_await_quantity", nextVariantState);
    await sendQuantityPicker({ phone, stateData: nextVariantState, lang, waSettings });
    return;
  }
  if (state === "wa_order_await_quantity") {
    const { parseQuantityChoice } = await import("../lib/waPremiumJourney.js");
    const { sendQuantityPicker } = await import("../lib/waPremiumUi.js");
    const qty = parseQuantityChoice(trimmed);
    if (qty == null || qty < 1) {
      if (stateData.awaitingCustomQty) {
        const customN = Number.parseInt(trimmed.replace(/[^\d]/g, ""), 10);
        if (customN >= 1 && customN <= 99) {
          await proceedAfterQuantityPick(phone, stateData, customN, waSettings);
          return;
        }
      }
      stateData.awaitingCustomQty = /custom|apni|other/i.test(trimmed);
      const lang = (await import("../lib/waPremiumJourney.js")).resolveWaLang(stateData);
      await sendQuantityPicker({ phone, stateData, lang, waSettings });
      return;
    }
    const product = stateData.product;
    const variant = stateData.variant;
    if (!product || !variant) {
      ...
      await sendWaText(phone, "Product detail missing ho gayi. Product naam bhej kar dobara start karein.", waSettings);
      return;
    }
    await logWaProcessingStep({ phone, step: "commerce_quantity_selected", detail: "Customer selected quantity.", payload: { qty } });
    await proceedAfterQuantityPick(phone, stateData, qty, waSettings);
    return;
  }
  const {
    WA_ORDER_AWAIT_BANK_SCREENSHOT,
    ...
    WA_ORDER_AWAIT_ADDRESS_DETAIL,
    WA_ORDER_AWAIT_ADDRESS_EXTRAS,
    resolveWaLang: resolveWaLangJ,
    buildPaymentScreenshotAck,
    buildAddressPrompt,
  } = await import("../lib/waPremiumJourney.js");
  if (state === WA_ORDER_AWAIT_BANK_SCREENSHOT || state === WA_ORDER_AWAIT_EASYPAISA_SCREENSHOT) {
    ...
    const { sendBankPaymentActions } = await import("../lib/waPremiumUi.js");
    await sendBankPaymentActions(phone, lang, waSettings);
    return;
  }
  ...
  if (state === "wa_order_await_name") {
    const { resolveWaLang, buildPhonePrompt } = await import("../lib/waPremiumJourney.js");
    const { sendNamePicker } = await import("../lib/waPremiumUi.js");
    const lang = resolveWaLang(stateData);
    if (!stateData.awaitingNameText) {
      await sendNamePicker({ phone, contactName: stateData.waContactName, lang, waSettings });
      return;
    }
    ...
    stateData.awaitingNameText = false;
  ...
  if (state === "wa_order_await_phone") {
    ...
    const lang = resolveWaLang(stateData);
    if (!stateData.awaitingCustomPhone) {
      await sendInteractiveButtons({
        phone,
        text: lang === "en" ? "Confirm phone with buttons 👇" : "Phone confirm karein 👇",
        buttons: [
          { id: "wa_phone_same", title: "✅ Use This Number" },
          { id: "wa_phone_other", title: "✏️ Change Number" },
        ],
    ...
    if (!isValidCheckoutPhone(customerPhone)) {
      await sendInteractiveList({
        phone,
        body: lang === "en" ? "Valid phone in next message, or tap:" : "Valid number likh dein ya button:",
        buttonLabel: "Phone",
        rows: [{ id: "wa_phone_same", title: "✅ Use WhatsApp No" }],
        settings: waSettings,
        templateName: "phone_retry",
      });
      return;
    }
  ...
  if (state === WA_ORDER_AWAIT_ADDRESS_EXTRAS) {
    if (!stateData.landmark && trimmed.length >= 3 && !/^skip|no|nahi$/i.test(trimmed)) {
      stateData.landmark = trimmed;
    }
    if (!stateData.postalCode && /^\d{4,6}$/.test(trimmed)) {
      stateData.postalCode = trimmed;
    }
    await sendPaymentMethodButtons(phone, stateData, waSettings);
    return;
  }
  ...
  if (state === "wa_order_await_confirm") {
    ...
    if (!isCommerceConfirmationText(lower)) {
      const { sendOrderConfirmButtons } = await import("../lib/waPremiumUi.js");
      const { buildPremiumOrderSummary } = await import("../lib/waPremiumJourney.js");
      const lang = resolveWaLangJ(stateData, stateData.lastUserMessage ?? "");
      await sendOrderConfirmButtons(phone, buildPremiumOrderSummary(normalizeCommerceTotals(stateData), lang), waSettings);
      return;
    }
...
- If [CONVERSATION MEMORY] shows an active order flow or selected product, continue that flow — do not restart with greeting.`;
    ...
    await attachQuickActions({
      ...
      stateData: {
        ...
        pendingProductQuery: sessionMemory.selectedProductName ?? intent.productQuery,
        pendingEducationQuery: (sessionMemory.stateData as any)?.pendingEducationQuery,
      },
      currentState,
      context: resolveQuickActionContext({
        textBody,
        classified,
        intent: intent.intent,
        stateData: sessionMemory.stateData as Record<string, unknown>,
        currentState,
      }),
    }).catch(() => {});
