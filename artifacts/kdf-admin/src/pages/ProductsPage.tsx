    ...
    <div className="grid grid-cols-[1fr_88px_72px_100px_32px] gap-2 items-center py-2 px-3 border-b last:border-0 hover:bg-muted/20 transition-colors">
      <div className="flex items-center gap-2">
        ...
        <Input
          ...
          placeholder={isColor ? "e.g. Forest Green" : "e.g. 250GM"}
          value={variant.value}
...
export default function ProductsPage() {
  ...
  const handleOpenAdd = () => {
    setForm(emptyForm());
    setAutoVariationEnabled(false);
    setAutoVarConfig(defaultAutoVariationConfig());
  ...
  const handleOpenEdit = (p: any) => {
    ...
    const inferred = inferBaseFromVariants(variants);
    if (inferred) {
      const baseVariant = variants.find((v: ProductVariant) => v.name === "Weight");
      setAutoVarConfig({
        ...defaultAutoVariationConfig(),
        baseWeightGrams: inferred.baseGrams,
        basePrice: parseFloat(baseVariant?.price ?? p.price) || parseFloat(p.price) || 1200,
        baseStock: baseVariant?.stock ?? p.stock ?? 100,
        selectedGrams: inferred.weights,
      });
      setAutoVariationEnabled(true);
    } else {
      setAutoVariationEnabled(false);
      setAutoVarConfig(defaultAutoVariationConfig());
    }
    setVariantStockMode("individual");
    setEditingId(p.id);
    setActiveTab("basic");
    setIsOpen(true);
  };

  const applyAutoGeneratedVariants = (
    weightVariants: ProductVariant[],
    meta: { stockMode: VariantStockMode; baseStock: number }
  ) => {
    setVariantStockMode(meta.stockMode);
    const nonWeight = form.variants.filter((v) => v.name !== "Weight");
    const basePrice = weightVariants.find(
      (v) => parseWeightToGrams(v.value) === autoVarConfig.baseWeightGrams
    )?.price ?? weightVariants[0]?.price;

    setForm((f) => ({
      ...f,
      variants: [...nonWeight, ...weightVariants],
      price: basePrice ?? f.price,
      stock: meta.stockMode === "shared" ? meta.baseStock : weightVariants.reduce((s, v) => s + (v.stock || 0), 0),
    }));
    toast({
      title: "Variations generated",
      description: `${weightVariants.length} weight options · ${meta.stockMode === "shared" ? "shared" : "individual"} stock`,
    });
  };
  ...
  const displayVariantStock =
    form.variants.length > 0 && variantStockMode === "shared" && form.variants.every((v) => v.name === "Weight")
      ? autoVarConfig.baseStock
      : totalVariantStock;
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1.5">
                        ...
                        {form.variants.length > 0 && (
                          <p className="text-xs text-muted-foreground">
                            Stock from variations: <strong>{displayVariantStock}</strong>
                            {variantStockMode === "shared" && form.variants.every((v) => v.name === "Weight") && (
                              <span className="text-[#5FA800]"> (shared)</span>
                            )}
                          </p>
                        )}
                      </div>
                  ...
                  <TabsContent value="variations" className="mt-0 space-y-4">
                    <AutoWeightVariationPanel
                      ...
                      onEnabledChange={setAutoVariationEnabled}
                      config={{
                        ...autoVarConfig,
                        basePrice: autoVarConfig.basePrice || parseFloat(form.price) || 0,
                      }}
                      onConfigChange={(c) => {
                        setAutoVarConfig(c);
                        if (c.basePrice > 0 && !form.price) {
                          setForm((f) => ({ ...f, price: String(c.basePrice) }));
                        }
                      }}
                      productName={form.name}
                      onGenerate={applyAutoGeneratedVariants}
                    />
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {autoVariationEnabled
                            ? "Weight auto-generated above · add Color/Size manually below"
                    ...
                    {getGroups(form.variants).length === 0 && !autoVariationEnabled ? (
                      <div className="border-2 border-dashed rounded-xl p-10 text-center text-muted-foreground">
                        ...
                        <p className="text-xs mt-1 mb-4 max-w-xs mx-auto">
                          Enable <strong>Auto Generate</strong> above for weight, or add <strong>Color</strong> / <strong>Size</strong> groups manually
                        </p>
                        <Button type="button" variant="outline" size="sm" onClick={addVariationGroup}>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between text-xs text-muted-foreground pt-1 px-1">
                          ...
                          <span>Total stock: <strong className="text-foreground">{displayVariantStock}</strong></span>
                        </div>
                      </div>
                    )}
                  </TabsContent>
