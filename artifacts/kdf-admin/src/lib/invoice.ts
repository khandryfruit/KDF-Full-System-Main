export interface InvoiceOrder {
  ...
  loyaltyDiscount?: string | number;
  walletDiscount?: string | number;
  total: string | number;
...
function rs(n: string | number | undefined): string {
...
export function printInvoice(order: InvoiceOrder): void {
  ...
  const itemRows = items.map(item => `
    <tr>
      <td>${item.name}${item.variant ? ` <span class="variant">(${item.variant})</span>` : ''}</td>
      <td class="center">${item.qty}</td>
      <td class="right">${rs(item.price)}</td>
      <td class="right"><strong>${rs(Number(item.price) * item.qty)}</strong></td>
    </tr>
  `).join('');
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* Header */
    .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 36px; border-bottom: 3px solid #5FA800; padding-bottom: 24px; }
    .brand { display: flex; align-items: center; gap: 14px; }
    .logo { width: 52px; height: 52px; background: #5FA800; border-radius: 14px; display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 900; font-size: 18px; letter-spacing: -1px; }
    .brand-name { font-size: 26px; font-weight: 900; color: #0D2B00; }
    .brand-name span { color: #5FA800; }
    .brand-tagline { font-size: 11px; color: #777; margin-top: 2px; }
    .invoice-meta { text-align: right; }
    .invoice-title { font-size: 22px; font-weight: 900; color: #5FA800; text-transform: uppercase; letter-spacing: 2px; }
    .invoice-num { font-family: 'Courier New', monospace; font-size: 15px; font-weight: 700; color: #333; margin-top: 4px; }
    .invoice-date { font-size: 11px; color: #888; margin-top: 4px; }
    .status-badge { display: inline-block; padding: 3px 10px; border-radius: 20px; font-size: 11px; font-weight: 700; background: #eef7e6; color: #5FA800; margin-top: 6px; text-transform: capitalize; }
    ...
    .info-box p { font-size: 13px; color: #333; margin-bottom: 3px; line-height: 1.5; }
  ...
  <div class="page">
    ...
    <div class="header">
      <div class="brand">
        ...
        <div>
          <div class="brand-name">KDF <span>NUTS</span></div>
    ...
    <div class="info-grid">
      ...
      <div class="info-box">
        ...
        <p><strong>Payment:</strong> ${(order.paymentMethod ?? 'COD').replace(/_/g, ' ').toUpperCase()}</p>
        ...
        <p><strong>Delivery:</strong> ${order.deliveryType === 'self' ? 'Self Pickup' : `Courier (${order.courier?.toUpperCase() ?? 'TCS'})`}</p>
    ...
    <div class="totals-wrap">
      <div class="totals">
        ...
        ${Number(order.discount) > 0 ? `<div class="total-row discount"><span>Discount</span><span>- ${rs(order.discount)}</span></div>` : ''}
    ...
    <!-- Footer -->
    <div class="footer">
      <div class="footer-note">KDF NUTS • support@kdfnuts.com • +92 300 123 4567</div>
