...
import { ShoppingBag, Eye, EyeOff, Facebook, Mail, Lock, User, Phone, ChevronRight, ChevronDown, Check, ArrowRight, ArrowLeft } from 'lucide-react';
import './_group.css';
...
export function LoginScreen() {
  ...
  const countries = [
    "Pakistan", "UK", "USA", "UAE", "Saudi Arabia", "Australia", "Canada"
  ];

  return (
    <div className="w-full h-screen min-h-[100dvh] bg-[#F8F9FB] flex flex-col font-sans relative overflow-hidden">
      
      {/* Header Area */}
      <div className="pt-16 pb-0 px-6 bg-white shadow-sm rounded-b-3xl z-20 shrink-0 relative">
        <div className="flex items-center justify-center gap-2 mb-8">
          <div className="w-10 h-10 bg-[#0D2B00] rounded-xl flex items-center justify-center shadow-md">
            <ShoppingBag className="w-5 h-5 text-[#5FA800]" />
          </div>
      ...
      <div className="flex-1 overflow-x-hidden overflow-y-auto relative z-10 w-full">
        ...
        <div className={`absolute top-0 left-0 w-full p-6 transition-all duration-500 ease-in-out ${activeTab === 'login' ? 'translate-x-0 opacity-100' : '-translate-x-full opacity-0 pointer-events-none'}`}>
          <div className="space-y-5">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Phone or Email</label>
            ...
            <button 
              className="w-full h-14 mt-6 rounded-xl text-white font-bold text-lg transition-transform active:scale-[0.98] shadow-lg shadow-[#5FA800]/30"
          ...
          <div className="mt-8 text-center pb-8">
            <button className="text-gray-500 font-bold hover:text-gray-800 transition-colors flex items-center justify-center gap-1 mx-auto text-sm">
              Continue as Guest 
        ...
        <div className={`absolute top-0 left-0 w-full transition-all duration-500 ease-in-out ${activeTab === 'signup' ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0 pointer-events-none'}`}>
          ...
          <div className="px-6 py-6 border-b border-gray-100 bg-white/50 backdrop-blur-sm sticky top-0 z-10">
            <div className="flex items-center justify-between max-w-[280px] mx-auto relative">
              ...
              <div className="flex flex-col items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors duration-300 ${signupStep >= 1 ? 'bg-[#5FA800] text-white shadow-md shadow-[#5FA800]/20' : 'bg-gray-100 text-gray-400'}`}>
                  1
                </div>
              ...
              <div className="flex flex-col items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300 border-2 ${signupStep === 2 ? 'bg-[#5FA800] border-[#5FA800] text-white shadow-md shadow-[#5FA800]/20' : 'bg-white border-gray-300 text-gray-400'}`}>
                  2
                </div>
          ...
          <div className="relative overflow-hidden min-h-[500px]">
            ...
            <div className={`absolute top-0 left-0 w-full p-6 transition-all duration-500 ease-in-out ${signupStep === 1 ? 'translate-x-0 opacity-100 relative' : '-translate-x-full opacity-0 absolute pointer-events-none'}`}>
              <div className="space-y-4">
                <div>
                  ...
                  <div className="relative flex items-center h-14 bg-white rounded-xl border border-gray-200 focus-within:border-[#5FA800] focus-within:ring-2 focus-within:ring-[#5FA800]/20 transition-all shadow-sm">
                    ...
                    <input 
                      ...
                      className="w-full h-full px-3 outline-none text-gray-800 bg-transparent font-medium" 
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative flex items-center h-14 bg-white rounded-xl border border-gray-200 focus-within:border-[#5FA800] focus-within:ring-2 focus-within:ring-[#5FA800]/20 transition-all overflow-hidden shadow-sm">
                    ...
                    <input 
                      ...
                      className="w-full h-full px-4 outline-none text-gray-800 bg-transparent font-medium" 
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className={`relative flex items-center h-14 bg-white rounded-xl border ${passwordMismatch ? 'border-red-400 focus-within:border-red-500 focus-within:ring-red-500/20' : 'border-gray-200 focus-within:border-[#5FA800] focus-within:ring-[#5FA800]/20'} focus-within:ring-2 transition-all shadow-sm`}>
                  ...
                  {passwordMismatch && (
                    <p className="text-red-500 text-sm font-medium mt-1.5 px-1 animate-in slide-in-from-top-1">Passwords do not match</p>
                  )}
                </div>
            ...
            <div className={`absolute top-0 left-0 w-full transition-all duration-500 ease-in-out ${signupStep === 2 ? 'translate-x-0 opacity-100 relative' : 'translate-x-full opacity-0 absolute pointer-events-none'}`}>
              <div className="p-6 pb-32 space-y-5">
                ...
                <div>
                  ...
                  <input 
                    ...
                    value={fullName}
                    onChange={(e) => setFullName(e.target.value)}
                    className="w-full h-14 px-4 bg-white rounded-xl border border-gray-200 outline-none text-gray-800 focus:border-[#5FA800] focus:ring-2 focus:ring-[#5FA800]/20 font-medium shadow-sm transition-all" 
                  />
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select 
                        ...
                        className="w-full h-14 pl-4 pr-10 bg-white rounded-xl border border-gray-200 outline-none text-gray-800 focus:border-[#5FA800] focus:ring-2 focus:ring-[#5FA800]/20 appearance-none font-medium shadow-sm transition-all"
                      >
                        {countries.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                      <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400 pointer-events-none" />
                      {country === 'Pakistan' && (
                        <div className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none bg-green-50 px-1.5 py-0.5 rounded text-[10px] font-bold text-green-700 hidden sm:block">PK</div>
                      )}
                    </div>
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    className="w-full h-24 p-4 bg-white rounded-xl border border-gray-200 outline-none text-gray-800 focus:border-[#5FA800] focus:ring-2 focus:ring-[#5FA800]/20 resize-none font-medium shadow-sm transition-all"
                  ></textarea>
                </div>
              ...
              {/* Sticky Bottom Bar */}
              <div className="fixed bottom-0 left-0 w-full bg-white border-t border-gray-100 p-4 pb-6 flex items-center gap-3 z-30 shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.1)]">
                <button 
                  onClick={() => setSignupStep(1)}
                  className="h-14 px-5 flex items-center justify-center rounded-xl font-bold text-gray-600 bg-gray-50 hover:bg-gray-100 transition-colors shrink-0"
                >
                  <ArrowLeft className="w-5 h-5 mr-1" /> Back
                </button>
                <button 
                  disabled={!step2Valid}
                  className={`flex-1 h-14 flex items-center justify-center rounded-xl text-white font-bold text-lg transition-all ${step2Valid ? 'bg-[#5FA800] active:scale-[0.98] shadow-lg shadow-[#5FA800]/30' : 'bg-gray-300 cursor-not-allowed opacity-50'}`}
                >
                  Create Account
                </button>
              </div>
