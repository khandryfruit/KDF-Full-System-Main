...
export function CategoriesPage() {
  ...
  return (
    <div className="w-full min-h-[100dvh] pb-20 relative font-sans" style={{ backgroundColor: BG_COLOR }}>
      {/* Header */}
      <header className="sticky top-0 z-40 bg-white shadow-sm px-4 py-3 flex items-center justify-between">
        <h1 className="text-xl font-bold text-gray-900">Categories</h1>
        <div className="flex items-center gap-3 text-gray-600">
          <button onClick={() => setLocation('/products')} className="p-2 hover:bg-gray-100 rounded-full transition-colors"><Search size={22} /></button>
        </div>
      </header>
      ...
      <main className="flex flex-col gap-6 pt-4 px-4 overflow-x-hidden">
        ...
        <section className="grid grid-cols-2 gap-3">
          <div onClick={() => setLocation('/products')} className="cursor-pointer h-28 rounded-2xl bg-gradient-to-br from-[#5FA800] to-green-700 p-4 relative overflow-hidden shadow-sm flex flex-col justify-center">
          ...
          <div onClick={() => setLocation('/products')} className="cursor-pointer h-28 rounded-2xl bg-gradient-to-br from-[#F58300] to-orange-600 p-4 relative overflow-hidden shadow-sm flex flex-col justify-center">
      ...
      {selectedCategory === 'electronics' && (
        <div className="fixed bottom-[68px] left-0 right-0 w-full bg-white border-t border-gray-100 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-30 animate-in slide-in-from-bottom-4 p-4 rounded-t-2xl">
          ...
          <div className="overflow-hidden -mx-4">
            <div className="flex gap-2 overflow-x-auto hide-scrollbar px-4 pb-2">
              {subCategories.map((sub, i) => (
                <button key={i} onClick={() => setLocation('/products')} className="flex-shrink-0 px-4 py-2 bg-gray-50 hover:bg-gray-100 rounded-full text-sm font-medium text-gray-700 transition-colors border border-gray-200">
                  {sub}
                </button>
              ))}
            </div>
